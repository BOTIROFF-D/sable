...
1. **Компиляция AST в замыкания** — по замерам это следующий крупный выигрыш (ещё ×1,5–2,5): узел один раз превращается в JS-функцию, и `switch` уходит из горячего цикла.
2. **Разрешение имён в слоты** — поиск по цепочке областей всё ещё 8–15% времени.
