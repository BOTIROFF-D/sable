...
selfhost/         Sable, написанный на Sable: лексер, парсер, интерпретатор
...
| `tests/selfhost.ts` | перёд на Sable даёт те же лексемы и то же дерево, что настоящий |
| `tests/selfrun.ts` | интерпретатор на Sable печатает то же, что настоящий, — на всех примерах |
...
Известное ограничение: глубина рекурсии. Порог языка — 900 вызовов, но реальная
ёмкость зависит от тела функции: от ~1180 вызовов при плоском теле до ~150 при
сорока уровнях вложенности выражений. Глубже — ошибка языка про вложенность
вычислений, не стек JavaScript. Порог поднимается через `SABLE_MAX_DEPTH`
вместе с `node --stack-size=...`; цифры и способ замера — в
[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
«Sable» — имя проекта, принадлежащее THE BOTIROFF LLC; знак ™ означает
заявленные права на название, регистрация не требуется. Выпускать форк под этим
