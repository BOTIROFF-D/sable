...
| Диагностика | позиция в исходнике, стрелка под виновником, стек вызовов, подсказки по опечаткам |
...
Сделано после 0.1: модули (`import … as`) и перехват ошибок (`try`/`catch`).

Ближайшие шаги, в порядке пользы:

1. **Разбор ошибок с восстановлением** — сейчас парсер останавливается на первой; лучше показывать все сразу.
2. **Байткод + стековая ВМ** — обход дерева заменяется компиляцией в инструкции, ×10–50 к скорости.
3. **Выборочный импорт** — `import "utils.dbgo" as { double, area }` вместо одного пространства имён.
4. **Целочисленный тип** — сейчас всё число с плавающей точкой, как в JS.
