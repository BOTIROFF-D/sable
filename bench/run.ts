...
function checkStackMargin(): boolean {
  ...
  // Проверяем не конкретное сообщение, а то, что наружу вышла ошибка ЯЗЫКА.
  //
  // Раньше здесь ждали именно «слишком глубокая рекурсия», и сторож печатал
  // «порядок» на функции без вложенных выражений — то есть на самом лёгком
  // случае. Ёмкость же зависит от тела: замер даёт ~1180 вызовов при плоском
  // теле и ~150 при сорока уровнях вложенности. При глубоком теле счётчик
  // до 900 не доживает, и это законно — важно, что пользователь получает
  // ошибку Sable, а не стек JavaScript.
  const свой = message.startsWith('слишком глубокая рекурсия');
  const вложенность = message.startsWith('слишком глубокая вложенность');
  const ok = свой || вложенность;
  process.stdout.write(
    ok
      ? `\nзапас стека: наружу вышла ошибка языка (${свой ? 'свой предел' : 'предел стека'}) — порядок\n`
      : `\nЗАПАС СТЕКА ИСЧЕРПАН: вместо ошибки языка получено «${message}».\n` +
